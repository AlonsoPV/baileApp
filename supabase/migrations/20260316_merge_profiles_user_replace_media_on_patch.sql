-- Fix merge_profiles_user media behavior for profile uploads.
-- When client sends full media array in p_patch.media, replace media instead of concatenating.
...
BEGIN
  ...
  ON CONFLICT (user_id) DO UPDATE SET
    ...
    media = CASE
      WHEN p_patch ? 'media' THEN COALESCE(EXCLUDED.media, '[]'::jsonb)
      ELSE profiles_user.media
    END,
