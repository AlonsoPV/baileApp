-- RSVP system: table, RLS, RPCs, and sync triggers
...
DO $$ BEGIN
  EXECUTE 'DROP POLICY IF EXISTS rsvp_select_own ON public.event_rsvp';
  EXECUTE 'DROP POLICY IF EXISTS rsvp_insert_own ON public.event_rsvp';
  EXECUTE 'DROP POLICY IF EXISTS rsvp_update_own ON public.event_rsvp';
  EXECUTE 'DROP POLICY IF EXISTS rsvp_delete_own ON public.event_rsvp';
END $$;
...
-- 3) Optional: add counters to events_date and rsvp list to profiles_user
ALTER TABLE IF EXISTS public.events_date
...
-- 4) Trigger functions to keep counts in sync
