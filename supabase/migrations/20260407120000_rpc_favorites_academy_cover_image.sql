-- Favoritos clase (academia): image_url alineado con explore (portada, media p1/cover, avatar).
...
classes_academy as (
  select
    ...
    coalesce(
      nullif(trim((
        select coalesce(nullif(trim(m.elem->>'url'), ''), nullif(trim(m.elem->>'path'), ''))
        from jsonb_array_elements(coalesce(pa.media, '[]'::jsonb)) with ordinality as m(elem, o)
        where coalesce(m.elem->>'slot', '') in ('p1', 'cover')
        limit 1
      )), ''),
      nullif(trim(pa.avatar_url), ''),
      nullif(trim(pa.portada_url), ''),
      nullif(trim((
        select coalesce(nullif(trim(m.elem->>'url'), ''), nullif(trim(m.elem->>'path'), ''))
        from jsonb_array_elements(coalesce(pa.media, '[]'::jsonb)) as m(elem)
        where coalesce(nullif(trim(m.elem->>'url'), ''), nullif(trim(m.elem->>'path'), '')) is not null
        limit 1
      )), '')
    ) as image_url,
    nullif(item.obj->>'fecha', '') as date_label,
