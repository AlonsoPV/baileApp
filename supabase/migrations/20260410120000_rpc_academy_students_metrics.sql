-- Gestión por alumnos (academia): listado, detalle y métricas globales.
-- Fuente de verdad: public.clase_asistencias.
-- Período: created_at::date en UTC.

drop function if exists public.rpc_get_academy_students_list(bigint, date, date, text, text, text, text, integer, integer);
drop function if exists public.rpc_get_academy_student_detail(bigint, uuid, date, date);
drop function if exists public.rpc_get_academy_students_global_metrics(bigint, date, date);
...
comment on function public.rpc_get_academy_students_list(bigint, date, date, text, text, text, text, integer, integer) is
'Listado de alumnos por academia agregado por user_id, con segmentación por estado/rol/zona y métricas de actividad.';
...
begin
  ...
  select jsonb_build_object(
      ...
      'total_attended', coalesce(count(*) filter (where e.status_norm in ('asistio', 'asistió', 'attended')), 0),
  ...
  with class_rows as (
    select
      ...
      count(*) filter (where lower(coalesce(trim(ca.status), 'unknown')) = 'tentative')::bigint as tentative,
      count(*) filter (where lower(coalesce(trim(ca.status), 'unknown')) = 'pagado')::bigint as paid,
...
comment on function public.rpc_get_academy_student_detail(bigint, uuid, date, date) is
'Detalle de un alumno por academia con métricas y trayectoria de clases basada en clase_asistencias.';
...
comment on function public.rpc_get_academy_students_global_metrics(bigint, date, date) is
'KPIs globales por alumnos (únicos, activos, nuevos, recurrentes, historial), con desglose por estado/rol/zona.';
