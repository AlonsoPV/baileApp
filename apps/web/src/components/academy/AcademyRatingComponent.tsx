...
import {
  useAcademyRatingStats,
  useMyAcademyRating,
  useUpsertAcademyRating,
  type RatingValue,
  getRatingDisplay,
  calculateAverage,
} from '../../hooks/useAcademyRatings';
import { useToast } from '../Toast';

interface AcademyRatingComponentProps {
  academyId: number;
}

const RATING_OPTIONS: RatingValue[] = ['excelente', 'muy_bueno', 'bueno', 'regular', 'no_aplica'];

const QUESTIONS = [
  {
    id: 'overall' as const,
    label: '¿Cómo calificarías a esta academia?',
    key: 'overall_rating' as const,
  },
  {
    id: 'puntualidad' as const,
    label: 'Puntualidad y cumplimiento',
    key: 'puntualidad' as const,
  },
  {
    id: 'instalaciones' as const,
    label: 'Instalaciones, limpieza y orden',
    key: 'instalaciones' as const,
  },
  {
    id: 'atencion_staff' as const,
    label: 'Atención y apoyo del staff/maestros',
    key: 'atencion_staff' as const,
  },
  {
    id: 'organizacion' as const,
    label: 'Variedad de horarios',
    key: 'organizacion' as const,
  },
  {
    id: 'precio_valor' as const,
    label: 'Relación precio–valor',
    key: 'precio_valor' as const,
  },
];
  ...
  React.useEffect(() => {
    if (myRating) {
      setFormData({
        ...
        instalaciones: myRating.instalaciones || null,
        atencion_staff: myRating.atencion_staff || null,
        organizacion: myRating.organizacion || null,
        precio_valor: myRating.precio_valor || null,
      });
      // Si ya tiene calificaciones adicionales, mostrar el panel expandido
      if (myRating.puntualidad || myRating.instalaciones || myRating.atencion_staff || myRating.organizacion || myRating.precio_valor) {
        setShowAdditionalQuestions(true);
      }
    }
  }, [myRating]);

  const handleRatingChange = (key: string, value: RatingValue) => {
    setFormData((prev) => ({
      ...prev,
      [key]: value,
    }));
  ...
  const handleSubmit = async () => {
    ...
    try {
      await upsertRating.mutateAsync({
        ...
        instalaciones: formData.instalaciones || null,
        atencion_staff: formData.atencion_staff || null,
      ...
      console.error('[AcademyRatingComponent] Error al guardar calificación:', error);
      showToast('Error al guardar la calificación', 'error');
    }
  };

  // Calcular promedio general
  const overallAverage = stats ? calculateAverage(stats.overall) : null;
    ...
    >
      <style>{`
        ...
        @media (max-width: 768px) {
          .academy-rating-container {
            padding: 1rem !important;
            border-radius: 16px !important;
            margin-bottom: 1.5rem !important;
          }
          
          .academy-rating-header {
          ...
          .academy-rating-header-content {
            flex-direction: row !important;
            gap: 1rem !important;
            width: 100%;
          }
          
          .academy-rating-avatar {
            width: 60px !important;
            height: 60px !important;
            font-size: 1.5rem !important;
          }
          
          .academy-rating-title {
            font-size: 1.1rem !important;
            margin-bottom: 0.25rem !important;
          }
          
          .academy-rating-button {
            width: 100% !important;
          ...
          .academy-rating-options {
            gap: 0.5rem !important;
          }
          
          .academy-rating-option {
            padding: 0.5rem 0.75rem !important;
            font-size: 0.8rem !important;
          }
          
          .academy-rating-option-icon {
            font-size: 1rem !important;
          }
          
          .academy-rating-breakdown {
            margin-top: 1.5rem !important;
            padding-top: 1.5rem !important;
          }
          
          .academy-rating-breakdown-title {
            font-size: 1rem !important;
            margin-bottom: 0.75rem !important;
          }
          
          .academy-rating-breakdown-grid {
            grid-template-columns: 1fr !important;
            gap: 0.75rem !important;
          }
          
          .academy-rating-breakdown-card {
            padding: 0.75rem !important;
          }
          
          .academy-rating-breakdown-label {
            font-size: 0.8rem !important;
            margin-bottom: 0.4rem !important;
          }
          
          .academy-rating-breakdown-value {
            font-size: 1.25rem !important;
          }
          
          .academy-rating-breakdown-count {
      ...
      {/* Header con promedio */}
      <div className="academy-rating-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '2rem' }}>
        <div className="academy-rating-header-content" style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
          <div
            className="academy-rating-avatar"
            style={{
              width: '80px',
              height: '80px',
              borderRadius: '50%',
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <h3 className="section-title academy-rating-title" style={{ margin: 0, marginBottom: '0.5rem' }}>
              ⭐ Calificaciones
            </h3>
            {overallAverage && (
        ...
        <div style={{ flexShrink: 0 }}>
          {user && (
            <motion.button
              ...
              className="academy-rating-button"
              style={{
        ...
        {isOpen && user && (
          ...
          >
            ...
            <div style={{ marginBottom: showAdditionalQuestions ? '1.5rem' : '0' }}>
              ...
              <div
                className="academy-rating-options"
                style={{
                  display: 'flex',
                  flexWrap: 'wrap',
                  gap: '1rem',
              ...
              >
                {RATING_OPTIONS.map((option) => {
                  const { emoji, label } = getRatingDisplay(option);
                  ...
                  return (
                    <motion.button
                      ...
                      className={`rating-option academy-rating-option ${isSelected ? 'selected' : ''}`}
                      style={{
                        padding: '0.6rem 1rem',
                        borderRadius: '10px',
                    ...
                    >
                      <span className="academy-rating-option-icon" style={{ fontSize: '1.1rem' }}>{emoji}</span>
                      <span>{label}</span>
                    </motion.button>
                  );
                })}
                        ...
                        >
                          {RATING_OPTIONS.map((option) => {
                            ...
                            return (
                              ...
                              >
                                <span className="academy-rating-option-icon" style={{ fontSize: '1rem' }}>{emoji}</span>
                                <span>{label}</span>
                              </motion.button>
                            );
                          })}
      ...
      {stats && totalRatings > 0 && (
        <div className="academy-rating-breakdown" style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid rgba(255, 255, 255, 0.1)' }}>
          <h4 className="academy-rating-breakdown-title" style={{ fontSize: '1.1rem', fontWeight: '600', marginBottom: '1rem', color: '#fff' }}>
            Desglose por categoría
          </h4>
          <div className="academy-rating-breakdown-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '1rem' }}>
            {QUESTIONS.map((question) => {
              const categoryStats = stats[question.id];
              const avg = calculateAverage(categoryStats);
              ...
              return (
                <div
                  key={question.id}
                  className="academy-rating-breakdown-card"
                  style={{
                    padding: '1rem',
                ...
                >
                  <div className="academy-rating-breakdown-label" style={{ fontSize: '0.85rem', fontWeight: '600', marginBottom: '0.5rem', color: '#fff' }}>
                    {question.label}
                  </div>
                  <div className="academy-rating-breakdown-value" style={{ fontSize: '1.5rem', fontWeight: '700', color: question.id === 'overall' ? '#22c55e' : '#22c55e' }}>
                    {avg ? avg.toFixed(1) : '—'}
                  </div>
                  <div className="academy-rating-breakdown-count" style={{ fontSize: '0.75rem', opacity: 0.7, color: '#fff', marginTop: '0.25rem' }}>
