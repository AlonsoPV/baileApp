...
export function ProfileCard({ profile, isOwnProfile, onEdit }: ProfileCardProps) {
    ...
    >
      ...
      <div style={{ position: 'relative', height: '600px', overflow: 'hidden' }}>
        ...
        >
          ...
          >
            {/* Ritmos Chips */}
            {getRitmoNombres().slice(0, 3).map((nombre, index) => (
              <motion.div
                key={`ritmo-${nombre}`}
                variants={chipVariants}
                whileHover={{ scale: 1.05, boxShadow: `0 0 20px ${colors.coral}aa` }}
                style={{
                  padding: '8px 16px',
                  borderRadius: '20px',
            ...
            {/* Zonas Chips */}
            {getZonaNombres().slice(0, 2).map((nombre, index) => (
              <motion.div
                key={`zona-${nombre}`}
                variants={chipVariants}
                whileHover={{ scale: 1.05, boxShadow: `0 0 20px ${colors.yellow}aa` }}
                style={{
                  padding: '8px 16px',
                  borderRadius: '20px',
            ...
            {isOwnProfile ? (
              <motion.button
                ...
                style={{
                  ...
                  fontWeight: '600',
                  cursor: 'pointer',
                  boxShadow: '0 4px 15px rgba(255,61,87,0.4)',
                }}
              >
                ✏️ Editar Perfil
              </motion.button>
