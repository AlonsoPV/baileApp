...
type CronoItem = {
  ...
  referenciaCosto?: string; // nombre o clave para cruzar con costos
};

type CostoItem = {
  nombre?: string;        // "Clase Individual", "Full Pass"
  tipo?: 'preventa' | 'taquilla' | 'promo' | string;
  precio?: number | null; // null => Gratis
  regla?: string;         // "Hasta el 15 de nov", etc.
};
...
interface Props {
  ...
  title?: string; // opcional (ej. "Clases & Talleres")
}
...
export default function TeacherSchedule({ date, title = 'Clases & Talleres' }: Props) {
  ...
  if (!enriched.length && !costos.length) return null;
            ...
            {costos.map((c, i) => (
              <div key={i} style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  {c.regla && (
                    <span style={{ fontSize: 12, color: 'rgba(255,255,255,0.7)' }}>{c.regla}</span>
                  )}
                </div>
