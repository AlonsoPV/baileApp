-- ============================================================================
-- 🔒 CORREGIR ERRORES DE SEGURIDAD DEL LINTER DE SUPABASE
-- ============================================================================
-- Este script corrige todos los errores de seguridad detectados:
-- 1. auth_users_exposed: Vista que expone auth.users
-- 2. security_definer_view: Vistas con SECURITY DEFINER
-- 3. rls_disabled_in_public: Tablas sin RLS habilitado
-- ============================================================================
-- Ejecutar en Supabase SQL Editor
-- ============================================================================

BEGIN;

-- ============================================================================
-- PARTE 1: CORREGIR VISTA QUE EXPONE auth.users
-- ============================================================================

-- 1.1 Eliminar vista problemática v_challenge_submissions_enriched si existe
DROP VIEW IF EXISTS public.v_challenge_submissions_enriched CASCADE;

-- 1.2 Recrear vista de forma segura (sin exponer auth.users directamente)
-- Si necesitas datos de usuario, úsalos desde profiles_user en lugar de auth.users
CREATE OR REPLACE VIEW public.v_challenge_submissions_enriched AS
SELECT 
    cs.id,
    cs.challenge_id,
    cs.user_id,
    cs.video_url,
    cs.caption,
    cs.status,
    cs.created_at,
    cs.updated_at,
    -- Usar profiles_user en lugar de auth.users para datos públicos
    pu.display_name,
    pu.avatar_url,
    -- Contar votos
    COUNT(cv.user_id)::int AS votes
FROM public.challenge_submissions cs
LEFT JOIN public.profiles_user pu ON pu.user_id = cs.user_id
LEFT JOIN public.challenge_votes cv ON cv.submission_id = cs.id
WHERE cs.status = 'approved'
GROUP BY cs.id, cs.challenge_id, cs.user_id, cs.video_url, cs.caption, 
         cs.status, cs.created_at, cs.updated_at, pu.display_name, pu.avatar_url;

-- Otorgar permisos
GRANT SELECT ON public.v_challenge_submissions_enriched TO anon, authenticated;

-- ============================================================================
-- PARTE 2: ELIMINAR SECURITY DEFINER DE VISTAS PÚBLICAS
-- ============================================================================
-- Las vistas deben usar SECURITY INVOKER (por defecto) para que respeten RLS

-- 2.1 v_challenge_leaderboard
DROP VIEW IF EXISTS public.v_challenge_leaderboard CASCADE;
CREATE OR REPLACE VIEW public.v_challenge_leaderboard AS
SELECT 
    s.challenge_id,
    s.id AS submission_id,
    s.user_id,
    COUNT(v.user_id)::int AS votes
FROM public.challenge_submissions s
LEFT JOIN public.challenge_votes v ON v.submission_id = s.id
WHERE s.status = 'approved'
GROUP BY s.challenge_id, s.id, s.user_id;

GRANT SELECT ON public.v_challenge_leaderboard TO anon, authenticated;

-- 2.2 v_academy_classes_public
DROP VIEW IF EXISTS public.v_academy_classes_public CASCADE;
CREATE OR REPLACE VIEW public.v_academy_classes_public AS
SELECT
    ac.id,
    ac.academy_id,
    ac.nombre,
    ac.descripcion,
    ac.nivel,
    ac.ritmos_seleccionados,
    ac.dia_semana,
    ac.hora_inicio,
    ac.hora_fin,
    ac.costo,
    ac.ubicacion,
    ac.cupo_maximo,
    ac.created_at,
    a.nombre_publico AS academy_nombre,
    a.avatar_url AS academy_avatar
FROM public.academy_classes ac
JOIN public.profiles_academy a ON a.id = ac.academy_id
WHERE ac.estado = 'activo'
  AND a.estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_academy_classes_public TO anon, authenticated;

-- 2.3 v_academies_public
DROP VIEW IF EXISTS public.v_academies_public CASCADE;
CREATE OR REPLACE VIEW public.v_academies_public AS
SELECT 
    id,
    user_id,
    nombre_publico,
    bio,
    media,
    avatar_url,
    portada_url,
    ritmos,
    ritmos_seleccionados,
    zonas,
    redes_sociales,
    ubicaciones,
    horarios,
    cronograma,
    costos,
    estado_aprobacion,
    created_at,
    updated_at
FROM public.profiles_academy
WHERE estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_academies_public TO anon, authenticated;

-- 2.4 academies_live (alias de v_academies_public)
DROP VIEW IF EXISTS public.academies_live CASCADE;
CREATE OR REPLACE VIEW public.academies_live AS
SELECT * FROM public.v_academies_public;

GRANT SELECT ON public.academies_live TO anon, authenticated;

-- 2.5 profiles_user_light
DROP VIEW IF EXISTS public.profiles_user_light CASCADE;
CREATE OR REPLACE VIEW public.profiles_user_light AS
SELECT 
    user_id,
    display_name,
    avatar_url,
    ritmos,
    zonas,
    bio,
    created_at
FROM public.profiles_user
WHERE display_name IS NOT NULL 
  AND display_name != '';

GRANT SELECT ON public.profiles_user_light TO anon, authenticated;

-- 2.6 v_brands_public
DROP VIEW IF EXISTS public.v_brands_public CASCADE;
CREATE OR REPLACE VIEW public.v_brands_public AS
SELECT *
FROM public.profiles_brand
WHERE estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_brands_public TO anon, authenticated;

-- 2.7 events_live
DROP VIEW IF EXISTS public.events_live CASCADE;
CREATE OR REPLACE VIEW public.events_live AS
SELECT 
    ed.id,
    ed.parent_id,
    ed.nombre,
    ed.biografia,
    ed.fecha,
    ed.hora_inicio,
    ed.hora_fin,
    ed.lugar,
    ed.direccion,
    ed.ciudad,
    ed.zona,
    ed.referencias,
    ed.requisitos,
    ed.cronograma,
    ed.costos,
    ed.media,
    ed.flyer_url,
    ed.created_at,
    ed.updated_at,
    ep.nombre AS evento_nombre,
    ep.descripcion AS evento_descripcion,
    ep.biografia AS evento_biografia,
    ep.estilos AS evento_estilos,
    ep.zonas AS evento_zonas,
    ep.sede_general,
    ep.faq,
    ep.media AS evento_media,
    po.id AS organizador_id,
    po.user_id AS organizador_user_id,
    po.nombre_publico AS organizador_nombre,
    po.bio AS organizador_bio,
    po.media AS organizador_media,
    po.ritmos AS organizador_ritmos,
    po.ritmos_seleccionados AS organizador_ritmos_seleccionados,
    po.zonas AS organizador_zonas
FROM public.events_date ed
JOIN public.events_parent ep ON ed.parent_id = ep.id
JOIN public.profiles_organizer po ON ep.organizer_id = po.id
WHERE po.estado_aprobacion = 'aprobado'
  AND ed.fecha >= CURRENT_DATE;

GRANT SELECT ON public.events_live TO anon, authenticated;

-- 2.8 organizers_live
DROP VIEW IF EXISTS public.organizers_live CASCADE;
CREATE OR REPLACE VIEW public.organizers_live AS
SELECT 
    po.id,
    po.user_id,
    po.nombre_publico,
    po.bio,
    po.media,
    po.ritmos,
    po.ritmos_seleccionados,
    po.zonas,
    po.redes_sociales,
    po.faq,
    po.created_at,
    po.updated_at,
    po.estado_aprobacion
FROM public.profiles_organizer po
WHERE po.estado_aprobacion = 'aprobado';

GRANT SELECT ON public.organizers_live TO anon, authenticated;

-- 2.9 v_events_dates_public
DROP VIEW IF EXISTS public.v_events_dates_public CASCADE;
CREATE OR REPLACE VIEW public.v_events_dates_public AS
SELECT 
    ed.*,
    ep.nombre AS evento_nombre,
    ep.organizer_id
FROM public.events_date ed
JOIN public.events_parent ep ON ed.parent_id = ep.id
JOIN public.profiles_organizer po ON ep.organizer_id = po.id
WHERE po.estado_aprobacion = 'aprobado'
  AND ed.fecha >= CURRENT_DATE;

GRANT SELECT ON public.v_events_dates_public TO anon, authenticated;

-- 2.10 v_events_parent_public
DROP VIEW IF EXISTS public.v_events_parent_public CASCADE;
CREATE OR REPLACE VIEW public.v_events_parent_public AS
SELECT 
    ep.*
FROM public.events_parent ep
JOIN public.profiles_organizer po ON ep.organizer_id = po.id
WHERE po.estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_events_parent_public TO anon, authenticated;

-- 2.11 v_teachers_public
DROP VIEW IF EXISTS public.v_teachers_public CASCADE;
CREATE OR REPLACE VIEW public.v_teachers_public AS
SELECT *
FROM public.profiles_teacher
WHERE estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_teachers_public TO anon, authenticated;

-- 2.12 v_user_public
DROP VIEW IF EXISTS public.v_user_public CASCADE;
CREATE OR REPLACE VIEW public.v_user_public AS
SELECT 
    user_id,
    display_name,
    avatar_url,
    ritmos,
    zonas,
    bio,
    redes_sociales,
    created_at
FROM public.profiles_user
WHERE display_name IS NOT NULL 
  AND display_name != '';

GRANT SELECT ON public.v_user_public TO anon, authenticated;

-- 2.13 v_teacher_classes_public
DROP VIEW IF EXISTS public.v_teacher_classes_public CASCADE;
CREATE OR REPLACE VIEW public.v_teacher_classes_public AS
SELECT
    tc.id,
    tc.teacher_id,
    tc.nombre,
    tc.descripcion,
    tc.nivel,
    tc.ritmos_seleccionados,
    tc.dia_semana,
    tc.hora_inicio,
    tc.hora_fin,
    tc.costo,
    tc.ubicacion,
    tc.cupo_maximo,
    tc.created_at,
    t.nombre_publico AS teacher_nombre,
    t.avatar_url AS teacher_avatar
FROM public.teacher_classes tc
JOIN public.profiles_teacher t ON t.id = tc.teacher_id
WHERE tc.estado = 'activo'
  AND t.estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_teacher_classes_public TO anon, authenticated;

-- 2.14 v_organizers_public
DROP VIEW IF EXISTS public.v_organizers_public CASCADE;
CREATE OR REPLACE VIEW public.v_organizers_public AS
SELECT *
FROM public.profiles_organizer
WHERE estado_aprobacion = 'aprobado';

GRANT SELECT ON public.v_organizers_public TO anon, authenticated;

-- 2.15 v_user_roles
DROP VIEW IF EXISTS public.v_user_roles CASCADE;
CREATE OR REPLACE VIEW public.v_user_roles AS
SELECT 
    ur.user_id,
    ur.role_slug,
    ur.granted_at AS created_at
FROM public.user_roles ur;

GRANT SELECT ON public.v_user_roles TO authenticated;

-- 2.16 v_academy_accepted_teachers
DROP VIEW IF EXISTS public.v_academy_accepted_teachers CASCADE;
CREATE OR REPLACE VIEW public.v_academy_accepted_teachers AS
SELECT 
    ati.academy_id,
    ati.teacher_id,
    ati.status,
    ati.created_at,
    ati.invited_at,
    ati.responded_at,
    pt.nombre_publico AS teacher_nombre,
    pt.avatar_url AS teacher_avatar
FROM public.academy_teacher_invitations ati
JOIN public.profiles_teacher pt ON pt.id = ati.teacher_id
WHERE ati.status = 'accepted';

GRANT SELECT ON public.v_academy_accepted_teachers TO anon, authenticated;

-- 2.17 v_teacher_academies
DROP VIEW IF EXISTS public.v_teacher_academies CASCADE;
CREATE OR REPLACE VIEW public.v_teacher_academies AS
SELECT 
    ati.teacher_id,
    ati.academy_id,
    ati.status,
    ati.created_at,
    ati.invited_at,
    ati.responded_at,
    pa.nombre_publico AS academy_nombre,
    pa.avatar_url AS academy_avatar
FROM public.academy_teacher_invitations ati
JOIN public.profiles_academy pa ON pa.id = ati.academy_id
WHERE ati.status = 'accepted';

GRANT SELECT ON public.v_teacher_academies TO anon, authenticated;

-- ============================================================================
-- PARTE 3: HABILITAR RLS EN TABLAS SIN PROTECCIÓN
-- ============================================================================

-- 3.1 admins (es una vista, no necesita RLS pero verificamos que esté bien)
-- La vista ya está definida arriba, solo verificamos que no exponga datos sensibles

-- 3.2 roles (tabla de catálogo - puede ser pública pero con RLS)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'public' AND table_name = 'roles') THEN
        ALTER TABLE public.roles ENABLE ROW LEVEL SECURITY;
        
        -- Política: lectura pública (es un catálogo)
        DROP POLICY IF EXISTS roles_select_public ON public.roles;
        CREATE POLICY roles_select_public ON public.roles
            FOR SELECT
            USING (true);
        
        -- Política: solo superadmins pueden modificar
        DROP POLICY IF EXISTS roles_modify_superadmin ON public.roles;
        CREATE POLICY roles_modify_superadmin ON public.roles
            FOR ALL
            USING (public.is_superadmin(auth.uid()))
            WITH CHECK (public.is_superadmin(auth.uid()));
    END IF;
END $$;

-- 3.3 ritmos_catalog (tabla de catálogo - puede ser pública pero con RLS)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'public' AND table_name = 'ritmos_catalog') THEN
        ALTER TABLE public.ritmos_catalog ENABLE ROW LEVEL SECURITY;
        
        -- Política: lectura pública (es un catálogo)
        DROP POLICY IF EXISTS ritmos_catalog_select_public ON public.ritmos_catalog;
        CREATE POLICY ritmos_catalog_select_public ON public.ritmos_catalog
            FOR SELECT
            USING (true);
        
        -- Política: solo superadmins pueden modificar
        DROP POLICY IF EXISTS ritmos_catalog_modify_superadmin ON public.ritmos_catalog;
        CREATE POLICY ritmos_catalog_modify_superadmin ON public.ritmos_catalog
            FOR ALL
            USING (public.is_superadmin(auth.uid()))
            WITH CHECK (public.is_superadmin(auth.uid()));
    END IF;
END $$;

-- 3.4 user_profiles (si existe como tabla separada)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'public' AND table_name = 'user_profiles') THEN
        ALTER TABLE public.user_profiles ENABLE ROW LEVEL SECURITY;
        
        -- Política: usuarios pueden ver su propio perfil
        DROP POLICY IF EXISTS user_profiles_select_own ON public.user_profiles;
        CREATE POLICY user_profiles_select_own ON public.user_profiles
            FOR SELECT
            USING (user_id = auth.uid());
        
        -- Política: usuarios pueden actualizar su propio perfil
        DROP POLICY IF EXISTS user_profiles_update_own ON public.user_profiles;
        CREATE POLICY user_profiles_update_own ON public.user_profiles
            FOR UPDATE
            USING (user_id = auth.uid())
            WITH CHECK (user_id = auth.uid());
        
        -- Política: usuarios pueden insertar su propio perfil
        DROP POLICY IF EXISTS user_profiles_insert_own ON public.user_profiles;
        CREATE POLICY user_profiles_insert_own ON public.user_profiles
            FOR INSERT
            WITH CHECK (user_id = auth.uid());
    END IF;
END $$;

-- 3.5 brand_products (si existe como tabla separada)
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'public' AND table_name = 'brand_products') THEN
        ALTER TABLE public.brand_products ENABLE ROW LEVEL SECURITY;
        
        -- Política: lectura pública si la marca está aprobada
        DROP POLICY IF EXISTS brand_products_select_public ON public.brand_products;
        CREATE POLICY brand_products_select_public ON public.brand_products
            FOR SELECT
            USING (
                EXISTS (
                    SELECT 1 FROM public.profiles_brand pb
                    WHERE pb.id = brand_products.brand_id 
                    AND pb.estado_aprobacion = 'aprobado'
                )
            );
        
        -- Política: solo el dueño de la marca puede modificar
        DROP POLICY IF EXISTS brand_products_modify_owner ON public.brand_products;
        CREATE POLICY brand_products_modify_owner ON public.brand_products
            FOR ALL
            USING (
                EXISTS (
                    SELECT 1 FROM public.profiles_brand pb
                    WHERE pb.id = brand_products.brand_id 
                    AND pb.user_id = auth.uid()
                )
            )
            WITH CHECK (
                EXISTS (
                    SELECT 1 FROM public.profiles_brand pb
                    WHERE pb.id = brand_products.brand_id 
                    AND pb.user_id = auth.uid()
                )
            );
    END IF;
END $$;

-- 3.6 event_prices (si existe como tabla separada)
DO $$
DECLARE
    table_exists boolean;
    column_exists boolean;
    fk_column_name text;
BEGIN
    -- Verificar si la tabla existe
    SELECT EXISTS (
        SELECT 1 FROM information_schema.tables 
        WHERE table_schema = 'public' AND table_name = 'event_prices'
    ) INTO table_exists;
    
    IF table_exists THEN
        -- Verificar qué columna de foreign key existe
        SELECT EXISTS (
            SELECT 1 FROM information_schema.columns
            WHERE table_schema = 'public' 
            AND table_name = 'event_prices' 
            AND column_name = 'event_date_id'
        ) INTO column_exists;
        
        IF column_exists THEN
            ALTER TABLE public.event_prices ENABLE ROW LEVEL SECURITY;
            
            -- Política: lectura pública si el evento está publicado
            DROP POLICY IF EXISTS event_prices_select_public ON public.event_prices;
            CREATE POLICY event_prices_select_public ON public.event_prices
                FOR SELECT
                USING (
                    EXISTS (
                        SELECT 1 FROM public.events_date ed
                        JOIN public.events_parent ep ON ed.parent_id = ep.id
                        JOIN public.profiles_organizer po ON ep.organizer_id = po.id
                        WHERE ed.id = event_prices.event_date_id
                        AND po.estado_aprobacion = 'aprobado'
                        AND ed.fecha >= CURRENT_DATE
                    )
                );
            
            -- Política: solo el organizador puede modificar
            DROP POLICY IF EXISTS event_prices_modify_organizer ON public.event_prices;
            CREATE POLICY event_prices_modify_organizer ON public.event_prices
                FOR ALL
                USING (
                    EXISTS (
                        SELECT 1 FROM public.events_date ed
                        JOIN public.events_parent ep ON ed.parent_id = ep.id
                        JOIN public.profiles_organizer po ON ep.organizer_id = po.id
                        WHERE ed.id = event_prices.event_date_id
                        AND po.user_id = auth.uid()
                    )
                )
                WITH CHECK (
                    EXISTS (
                        SELECT 1 FROM public.events_date ed
                        JOIN public.events_parent ep ON ed.parent_id = ep.id
                        JOIN public.profiles_organizer po ON ep.organizer_id = po.id
                        WHERE ed.id = event_prices.event_date_id
                        AND po.user_id = auth.uid()
                    )
                );
        ELSE
            -- Si la tabla existe pero no tiene event_date_id, habilitar RLS con política básica
            ALTER TABLE public.event_prices ENABLE ROW LEVEL SECURITY;
            
            -- Política básica: solo lectura pública (ajustar según necesidad)
            DROP POLICY IF EXISTS event_prices_select_public ON public.event_prices;
            CREATE POLICY event_prices_select_public ON public.event_prices
                FOR SELECT
                USING (true);
            
            -- Política básica: solo autenticados pueden modificar (ajustar según necesidad)
            DROP POLICY IF EXISTS event_prices_modify_authenticated ON public.event_prices;
            CREATE POLICY event_prices_modify_authenticated ON public.event_prices
                FOR ALL
                USING (auth.uid() IS NOT NULL)
                WITH CHECK (auth.uid() IS NOT NULL);
        END IF;
    END IF;
END $$;

-- 3.7 organizer_locations
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables 
               WHERE table_schema = 'public' AND table_name = 'organizer_locations') THEN
        ALTER TABLE public.organizer_locations ENABLE ROW LEVEL SECURITY;
        
        -- Política: lectura pública si el organizador está aprobado
        DROP POLICY IF EXISTS organizer_locations_select_public ON public.organizer_locations;
        CREATE POLICY organizer_locations_select_public ON public.organizer_locations
            FOR SELECT
            USING (
                EXISTS (
                    SELECT 1 FROM public.profiles_organizer po
                    WHERE po.id = organizer_locations.organizer_id
                    AND po.estado_aprobacion = 'aprobado'
                )
            );
        
        -- Política: solo el organizador puede modificar
        DROP POLICY IF EXISTS organizer_locations_modify_owner ON public.organizer_locations;
        CREATE POLICY organizer_locations_modify_owner ON public.organizer_locations
            FOR ALL
            USING (
                EXISTS (
                    SELECT 1 FROM public.profiles_organizer po
                    WHERE po.id = organizer_locations.organizer_id
                    AND po.user_id = auth.uid()
                )
            )
            WITH CHECK (
                EXISTS (
                    SELECT 1 FROM public.profiles_organizer po
                    WHERE po.id = organizer_locations.organizer_id
                    AND po.user_id = auth.uid()
                )
            );
    END IF;
END $$;

COMMIT;

-- ============================================================================
-- VERIFICACIÓN FINAL
-- ============================================================================

-- Verificar que las vistas no tienen SECURITY DEFINER
SELECT 
    schemaname,
    viewname,
    definition
FROM pg_views
WHERE schemaname = 'public'
  AND viewname IN (
    'v_challenge_leaderboard',
    'v_academy_classes_public',
    'v_academies_public',
    'academies_live',
    'profiles_user_light',
    'v_brands_public',
    'events_live',
    'organizers_live',
    'v_events_dates_public',
    'v_events_parent_public',
    'v_teachers_public',
    'v_user_public',
    'v_teacher_classes_public',
    'v_organizers_public',
    'v_user_roles',
    'v_academy_accepted_teachers',
    'v_teacher_academies',
    'v_challenge_submissions_enriched'
  )
ORDER BY viewname;

-- Verificar que las tablas tienen RLS habilitado
SELECT 
    schemaname,
    tablename,
    rowsecurity AS rls_enabled
FROM pg_tables
WHERE schemaname = 'public'
  AND tablename IN (
    'roles',
    'ritmos_catalog',
    'user_profiles',
    'brand_products',
    'event_prices',
    'organizer_locations'
  )
ORDER BY tablename;

-- Verificar políticas RLS creadas
SELECT 
    schemaname,
    tablename,
    policyname,
    permissive,
    roles,
    cmd
FROM pg_policies
WHERE schemaname = 'public'
  AND tablename IN (
    'roles',
    'ritmos_catalog',
    'user_profiles',
    'brand_products',
    'event_prices',
    'organizer_locations'
  )
ORDER BY tablename, policyname;

